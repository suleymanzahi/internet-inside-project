...
def tx(radio: RF24, address: bytes, conn: Connection) -> None:   # count = 5 will only transmit 5 packets
    ...
    while True:
        ...
        if not result:
            ...
            print(
                ...
                f"{(end_timer - start_timer) / 1000} us.", f"succesful packets: {i}"
            )
            # print(
            #     "Transmission successful! Time to Transmit:",
            #     f"{(end_timer - start_timer) / 1000} us. Sent: {payload.decode()}", f"succesful packets: {i}"
            # )


def rx(radio: RF24, address: bytes, conn: Connection):
